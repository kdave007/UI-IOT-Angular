...
export class ExternalSourceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subscription = this.range.getRange().subscribe(myRange => {
      ...
      this.voltageSub = this.voltageService.setNewValues(myRange,true).subscribe( data => {
        console.log("VOLTAGE RAW DATA",data)
        if(data.length){
          ...
          this.samples = data;
          this.sortData(this.samples);
          console.log("AFTER PROCESS",this.samples)
      ...
      // this.powBankSub = this.powerBankResolverService.setNewValues(myRange).subscribe( data => {
      ...
      if(i>0){
        if(pastTStamp==rows[i].timestamp){
         //ignore
          ...
          this.seriesLabel.push(new Date(rows[i].timestamp));
        }
      }
  ...
  private createChart(){
   // let mergedTsLabels = [...this.seriesLabel,...this.series_2_Label].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    ...
    this.data = {
      labels: this.seriesLabel,
      datasets: [{
        data: this.series,
        fill:false,
        label: 'Voltaje alimentación externa',
          ...
          tooltips: {
            ...
            callbacks: {
              ...
              label: function(tooltipItem, data) {
                ...
                document.getElementById('tooltipExternalSource').textContent = label;
