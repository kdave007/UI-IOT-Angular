...
export class NbPasswordAuthStrategy extends NbAuthStrategy {
  ...
  authenticate(data?: any): Observable<NbAuthResult> {
    ...
    return this.http.request(method, url, {withCredentials: true,body: data, observe: 'response'})
      .pipe(
        map((res) => {
    ...
    return observableOf({})
      .pipe(
        switchMap((res: any) => {
          ...
          return this.http.request(method, url, {observe: 'response',withCredentials: true});
        }),
        map((res) => {
          if (this.getOption(`${module}.alwaysFail`)) {
            throw this.createFailResponse();
